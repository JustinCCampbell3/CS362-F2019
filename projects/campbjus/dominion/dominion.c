...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mine(state, choice1, choice2, currentPlayer);
    ...
    case baron:
        return baron(state, choice1, currentPlayer);
    ...
    case minion:
        return minion(state, choice1, choice2, currentPlayer, handPos);
    ...
    case tribute:
        return tribute(state, currentPlayer, nextPlayer, tributeRevealedCards);

    case ambassador:
        return ambassador(state, choice1, choice2, currentPlayer);
...
int minion(struct gameState *state, int choice1, int choice2, int currentPlayer, int handPos){
      //+1 action
      state->numActions++;
      ...
      else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
      {
          ...
          for (i = 0; i < state->numPlayers; i++)
          {
              if (i != currentPlayer)
              {
                  ...
                  {
                      //discard hand
                      while( state->handCount[i] > 1 ) //BUG doesnt discard all cards in hand, should be 0 not 1
                      {
                          discardCard(handPos, i, state, 0);
                      }

                      //draw 4
                      for (j = 0; j < 3; j++) //BUG should be <4 and not 3; gives illusion that its a fresh hand cause the user does not draw an extra card
                      {
                          drawCard(i, state);
                      }
                  }
...
int tribute(struct gameState *state, int currentPlayer, int nextPlayer, int *tributeRevealedCards){
      ...
      for (i = 0; i <= 2; i ++) {
          if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold) { //Treasure cards
              state->coins += 3; //BUG gives the user an extra coin, sould be 2 and not 3
          }
              ...
              state->numActions = state->numActions + 1; //BUG should be 2 and not 1
          }
      }

      return 0;
}
int mine(struct gameState *state, int choice1, int choice2, int currentPlayer){
      j = state->hand[currentPlayer][choice1];  //store card we will trash
      ...
      if ( (getCost(state->hand[currentPlayer][choice1]) + 4) > getCost(choice2) ) //BUG should be 3 and not 4, lets user gain a card more expensie than is allowed
      {
          return -1;
      }
